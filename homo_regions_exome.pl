...
my $pan=1;
if ( !defined($panelname) || $panelname eq ''){$pan=0;}
...
our @inputdata = ();   # 2D array for CSV data
my $csv = Text::CSV->new;
...
while (<$in>){
    ...
    if ($info[0] =~ /#/){
        ...
        my @zygosity = map $_->[ 5 ], @inputdata[max($k-($window-1),0)..min($k+($window-1),$j)]; # taking het/hom info for variants $window before to $window after the one analyzed
        my @chromosome = map $_->[ 0 ], @inputdata[max($k-($window-1),0)..min($k+($window-1),$j)]; # same but for chromosome
        ...
        # if less than $windowthres variants before and after the one analyzed are homoz. we will stop analysis for this one
        if ($score<=$windowthres-1){$no=1;} 
        # taking a $window-variant window , sliding it arround the variant and taking best number of homoz. variant in score_max
        for (my $p =0; $p<$window; $p++){
            my @zygosity = map $_->[ 5 ], @inputdata[max($k-($window-1)+$p,0)..min($k+$p,$j)];
            my @chromosome = map $_->[ 0 ], @inputdata[max($k-($window-1)+$p,0)..min($k+$p,$j)];
            $score = 0;
