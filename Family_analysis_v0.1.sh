...
if [ -s $panel ]; then
    perl $here/Scripts/gene_names.pl $res/$out/$out.HomRegions.tsv $res/$out/$out.HomRegions.$panelname.tsv $panel $panelname 2> $here.log
fi 

# produce output pdf file
size=$(more $file | grep INFO | awk -F " " '{print $3}' )
outputR=$res/$out/$out.HomRegions.pdf
Rscript $here/Scripts/make_graph.R $out $file $outputR $size 2> $here.log
